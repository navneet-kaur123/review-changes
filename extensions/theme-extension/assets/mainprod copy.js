...
async function displayRating() {
  ...
  try {
    ...
    if (responseData) {
      if (productIdInput) {
        if (responseData.maintotalCount > 0) {
          if (responseData.maintotalCount > 1) {
            review = "Reviews";
          }
          let mainprodrev = document.querySelectorAll(".main-prdouct-star")[0];

          const starForMainProduct = `<ul class="card_product_star" onclick="scrollToBottom(0)">${generateStarElements(responseData.mainProdRating)}</ul>
                              <span class="card_product_rating">${responseData.mainProdRating.toFixed(1)} (${responseData.maintotalCount} ${review})</span>
                            </div>
                          `;
